        ...
        {ratings.map((rating) => (
          <div key={rating.value} className="flex flex-col items-center space-y-2">
            <div className="flex items-center space-x-2">
              <RadioGroupItem value={rating.value} id={`${questionId}-${rating.value}`} />
              <Label htmlFor={`${questionId}-${rating.value}`} className="cursor-pointer">
                {rating.value}
              </Label>
            </div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden relative">
        {/* Header */}
        <div className="bg-gray-50 border-b border-gray-200 p-6 pt-12">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">Section {currentSection + 1} of {totalSections}</span>
        ...
        <div className="absolute top-0 left-0 right-0 bg-blue-500 text-white p-2 text-center text-sm">
          <div className="flex items-center justify-center space-x-4">
            ...
            <span>•</span>
            <span>Viewing as: {user?.firstName} {user?.lastName} ({user?.department || user?.jobRole || 'Employee'})</span>
            <span>•</span>

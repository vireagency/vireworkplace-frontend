...
export const useNotificationActions = () => {
  ...
  const handleBulkDelete = useCallback(async (notificationIds) => {
    ...
    try {
      ...
      if (successful > 0) {
        toast.success(`${successful} notification${successful > 1 ? 's' : ''} deleted`);
      }
      
      if (failed > 0) {
        toast.error(`${failed} notification${failed > 1 ? 's' : ''} failed to delete`);
      }
  ...
  const handleBulkMarkAsRead = useCallback(async (notificationIds) => {
    ...
    try {
      ...
      if (successful > 0) {
        toast.success(`${successful} notification${successful > 1 ? 's' : ''} marked as read`);
      }
      
      if (failed > 0) {
        toast.error(`${failed} notification${failed > 1 ? 's' : ''} failed to mark as read`);
      }

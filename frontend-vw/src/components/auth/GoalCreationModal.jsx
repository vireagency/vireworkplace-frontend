...
export function GoalCreationModal({
  ...
  useEffect(() => {
    ...
    if (initialData && isEditing) {
      ...
      const formData = {
        ...
        description:
          initialData.description || initialData.goalDescription || "",
        priority: initialData.priority || "Medium",
        deadline:
          initialData.deadline ||
          (initialData.targetDeadline
            ? new Date(initialData.targetDeadline).toISOString().split("T")[0]
            : ""),
        ...
        keyMetrics: initialData.keyMetrics || [
          { metric: "General Performance", target: "To be defined" },
        ],
      };
  ...
  const categories =
    selectedGoalType === "Company" ? companyCategories : teamCategories;
  ...
  const onFormSubmit = async (data) => {
    ...
    console.log("Team field:", data.team);
    console.log("Department field:", data.department);
    ...
    try {
      ...
      if (result.success) {
        if (result.local) {
          // Goal was saved locally due to backend issues
          toast.success(
            result.message ||
              "Goal saved locally! It will sync when the server is available."
          );
          console.log("📝 Goal saved locally:", result.data);
        } else {
          // Goal was saved to server successfully
          toast.success(
            isEditing
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6 pt-4">
          ...
          <div>
            <Label
              htmlFor="title"
              className="text-[14px] font-medium text-[#0d141c] mb-2 block"
            >
              Goal Title *
            </Label>
            <Input
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  // For team goals, also set the team field
                  if (selectedGoalType === "Team") {
                    setValue("team", value);
          ...
          <div>
            <p className="text-[12px] text-slate-600 mb-3">
              Enter current and target values. They will be combined as "Current
              → Target" in the goal card.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label
                  htmlFor="currentMetric"
                  className="text-[14px] font-medium text-[#0d141c] mb-2 block"
                >
                  Current Metric *
                </Label>
                <Input
                  id="currentMetric"
                  {...register("currentMetric", {
                    required: "Current metric is required",
                  })}
          ...
          <div>
            ...
            <Textarea
              ...
              {...register("successCriteria", {
                required: "Success criteria is required",
              })}
